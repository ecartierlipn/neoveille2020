...
See main() function for details - Python 2.7+, Python 3.5+
'''
...
class corpus:
    ...
    def retrieve_corpus(self,save=True):
        ...
        if self.datatype == 'rss':
            ...
            feeds = rssfeeds(self.rssfeeds,self.lang,self.lang_iso,self.ling_config, constraints={'type':"web"})
            feeds.retrieve_feeds(self.lastindexed,save)
...
class rssfeeds(corpus):
    def __init__(self, feedlist, lang, lang_iso,ling_config,constraints={}):
        "construit une instance avec les attributs de base"
        if constraints.type == 'web':
            self.feedlist = self.build_rss(feedlist)
        else :
    ...
    def build_rss(self,feedlist):
        ''' build rss feeds for web sites without it with google news tool
            www.femina.fr =>
            https://news.google.com/news/rss/search/section/q/site:www.femina.fr/?&hl=fr&gl=fr&scoring=n&num=100
        
        '''
        newfeedlist = {}
        for url in feedlist:
            url2 = 'https://news.google.com/news/rss/search/section/q/site:' + url + '/?&hl=fr&gl=fr&scoring=n&num=100'
            newfeedlist[url2] = feedlist[url]
        return newfeedlist
...
if __name__ == '__main__':
    ...
    # logger from fileconfig
    logging.config.fileConfig('config/logging_config.ini', defaults={'logfilename': 'log/corpus_crawl_and_analysis.py_' + os.path.basename(configfile)})
...
#    ling_pipeline['pipeline'] = [x.strip() for x in config['GENERAL']['ling_pipeline'].split(',')] # list of processes
    ...
    # SPACY+EXCLUSIONDICO
        ...
        ling_config['model']=config['SPACY']['model']
        ling_config['token_tags']=[x.strip() for x in config['SPACY']['token_tags'].split(',')]
        ling_config['exclusiondico']=load_exclusion_dico(__iso)
        from lib.spacy_client import get_nlp,check_server, check_model
        # check spacy server is running
        res = check_server(ling_config['spacy_server'])
        if res is False:
